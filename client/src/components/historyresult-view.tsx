...
import { getReport, getImages } from "@/lib/queryClient";
...
export default function ResultsView({ jobId, onNewAnalysis, onViewHistory }: ResultsViewProps) {
  ...
  const [imageData, setImageData] = useState<AnalysisResponse| null>(null);
  const [availableImages, setAvalaibleImages] = useState<AdImage[]>([]);
  ...
  useEffect(() => {
    ...
    const startReportPolling = async () => {
      try {
        ...
        const images = await getImages({ companyName: "blank", jobId: jobId });
        const report = await getReport({ companyName: "blank", jobId: jobId });
        
        //availableImages = images.images || [];  
        if (report || images) {
          setReportData(report);
          setAvalaibleImages(images.images|| []);
          setImageData(images);
          console.log('Report polling completed successfully');
        } else {
          setReportError('Report not available after 60 seconds. The analysis may still be processing.');
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      addTextLine(reportData.companyName.toUpperCase(), margin, 25);
      ...
      pdf.setFontSize(9);
      pdf.setTextColor(255, 255, 255);
      addTextLine(`Job ID: ${imageData?.jobId}`, margin, 43);
      ...
      const fileName = `${reportData.companyName.replace(/\s+/g, "_")}_Analysis_Report.pdf`;
      pdf.save(fileName);
